...
internal sealed class ReaderGroupRepository(BookyServerDbContext db) : IReaderGroupRepository
{
    public async Task<IReadOnlyList<ReaderGroup>> GetActiveAsync(CancellationToken cancellationToken) =>
        await db.ReaderGroups.AsNoTracking()
            .Where(group => group.IsActive)
            .Include(group => group.Members)
            .OrderBy(group => group.Name)
            .ToListAsync(cancellationToken);

    public Task<ReaderGroup?> GetByIdAsync(Guid groupId, CancellationToken cancellationToken) =>
        db.ReaderGroups.AsNoTracking()
            .Include(group => group.Members)
    ...
    public async Task<IReadOnlyList<ReaderGroupMemberDto>> GetMembersAsync(
        ...
        await (from member in db.GroupMemberships.AsNoTracking()
               join profile in db.Profiles.AsNoTracking() on member.UserId equals profile.UserId into profileJoin
               from profile in profileJoin.DefaultIfEmpty()
               where member.GroupId == groupId
               orderby member.JoinedAt
               select new ReaderGroupMemberDto(member.UserId, profile == null ? null : profile.FirstName, member.JoinedAt))
            .ToListAsync(cancellationToken);
        ...
        Guid groupId, Guid userId, CancellationToken cancellationToken)
    {
        ...
        var existing = await db.GroupMemberships.AnyAsync(
        ...
        var memberCount = await db.GroupMemberships.CountAsync(
    ...
    {
        var membership = await db.GroupMemberships.FindAsync([groupId, userId], cancellationToken);
